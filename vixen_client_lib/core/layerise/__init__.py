...
def set_monitor(window: Gtk.Window, monitor_id: int | None):
    if not monitor_id: monitor_id = 0
        
    GtkLayerShell.set_monitor(
        window,
        Gdk.Display.get_default().get_monitor(monitor_id)
    )

def set_exclusive_zone(window: Gtk.Window, auto: bool, exclusive_zone: int | None):
    if not exclusive_zone: exclusive_zone = 0
    if auto: GtkLayerShell.auto_exclusive_zone_enable(window)
    else: GtkLayerShell.set_exclusive_zone(window, exclusive_zone)

def set_level(window: Gtk.Window, level: Levels | None):
    if not level: level = Levels.background
...
def set_anchor_edges(window: Gtk.Window, anchor_edges: List[Edges] | None):
    if not anchor_edges: anchor_edges = [Edges.top, Edges.right, Edges.bottom, Edges.left]
...
def set_margins(window: Gtk.Window, margins: Margins | None):
    if not margins: margins = Margins()
    margins_attr = ['top', 'right', 'bottom', 'left']
